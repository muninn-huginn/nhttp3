...
impl Connection {
    pub fn new_client(remote_addr: String, server_name: String) -> Self {
        Self { remote_addr, server_name: Some(server_name), is_client: true, established: true, next_stream_id: 0 }
    }

    pub fn new_server(remote_addr: String) -> Self {
        Self { remote_addr, server_name: None, is_client: false, established: true, next_stream_id: 1 }
    }
}
...
impl Connection {
    ...
    fn remote_addr(&self) -> &str { &self.remote_addr }
    fn server_name(&self) -> Option<&str> { self.server_name.as_deref() }
    fn is_established(&self) -> bool { self.established }

    #[pyo3(signature = (error_code=0, reason=None))]
    fn close(&mut self, error_code: u64, reason: Option<String>) { self.established = false; }

    fn __repr__(&self) -> String {
        format!("Connection({}, {})", self.remote_addr, if self.is_client { "client" } else { "server" })
    }
}
