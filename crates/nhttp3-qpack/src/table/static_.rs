...
/// Returns (index, name, value) for all 99 entries.
pub fn static_table() -> &'static [(usize, &'static [u8], &'static [u8])] {
    &STATIC_TABLE
}
...
pub fn find(name: &[u8], value: &[u8]) -> Option<(usize, bool)> {
    ...
    for &(idx, n, v) in STATIC_TABLE.iter() {
        if n == name {
            if v == value {
...
static STATIC_TABLE: [(usize, &[u8], &[u8]); 99] = [
    (0, b":authority", b""),
    (1, b":path", b"/"),
    (2, b"age", b"0"),
...
mod tests {
    ...
    #[test]
    fn find_exact_match() {
        let (idx, has_value) = find(b":method", b"GET").unwrap();
        assert_eq!(idx, 17);
        assert!(has_value);
    }
    ...
    fn find_name_only_match() {
        let (idx, has_value) = find(b":method", b"PATCH").unwrap();
        assert_eq!(idx, 15); // first :method entry
        assert!(!has_value);
    }

    #[test]
    fn find_no_match() {
        assert!(find(b"x-custom-header", b"anything").is_none());
    }
