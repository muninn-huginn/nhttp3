...
```rust
// crates/nhttp3-quic/src/packet/header.rs — add impl blocks

use bytes::Buf;
use nhttp3_core::{ConnectionId, VarInt, Error as CoreError};
...
impl Header {
    ...
    ///
    /// `local_cid_len` is needed to parse short headers — the caller must know
    /// the expected connection ID length from connection state.
    ...
    fn parse_long(buf: &mut Bytes) -> Result<Self, PacketError> {
        if buf.remaining() < 6 {
            return Err(PacketError::Core(CoreError::BufferTooShort));
        }
        ...
        let pn_offset = if packet_type != LongPacketType::Retry {
            let offset_before_len = buf.remaining();
            let payload_length = VarInt::decode(buf)?.value();
            let pn_offset = buf.len(); // This needs to be calculated relative to original buffer
            // We'll compute pn_offset differently — see below
        ...
        // Reset and re-parse to get pn_offset right. Actually, let's track offset manually.
        // For now, we need to know the byte position in the original packet.
        // Since Bytes tracks remaining, pn_offset = original_len - buf.remaining()

        // Let me restructure this. The pn_offset tracking needs the original buffer length.
        // We'll handle this by returning the number of bytes consumed as pn_offset.

        todo!("restructure to track pn_offset properly")
    }
...
Wait — the `pn_offset` tracking requires knowing the start position of the buffer. Let me restructure the parse function to take a slice reference instead of consuming bytes, or to track offset explicitly.

Here's the corrected approach — parse takes `&[u8]` and returns the header plus bytes consumed:
